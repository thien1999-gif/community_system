			...
			<div class="container">
				...
				<div class="col-md-12 search-table-col"
					...
					<div class="form-group pull-right col-lg-4">
						<form action="searchTraineeByEmail" method="POST">
							<button class="btn btn-primary" type="submit"
								style="width: 100px; margin-right: 100px;" type="submit">Search</button>
							<input type="text" placeholder="Search by name typing here.."
								class="search form-control" name="searchNameTrainee">
						</form>
					</div>
					...
					<c:url value="/admin/ListTrainee" var="pagedLink">
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<c:remove var="trainee" />

								<c:if test="${!listTrainer.isEmpty()}">
									<c:forEach var="trainee" items="${pagedListHolder.pageList}">

										<tr>
											<td>${trainee.email}</td>
											<td>${trainee.fullName}</td>
											<td>${trainee.phone}</td>
											<td>${trainee.gender == 1? 'Male':'FeMale'}</td>
											<td>${trainee.facebook}</td>
											<td><a
												href="${pageContext.request.contextPath}/admin/detailTrainee?id=${trainee.email}"
												...
												style="margin-left: 5px;"> <i class="typcn typcn-cancel"
													style="font-size: 18px;">Ban account</i>
											</a></td>
